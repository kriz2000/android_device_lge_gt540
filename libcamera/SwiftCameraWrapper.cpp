...
static void ensureSwiftLibOpened()
{
    if (g_swiftLibHandle == NULL) {
        ...
        if (g_swiftLibHandle == NULL) {
            ...
            g_swiftOpenCameraHardware = (OpenCamFunc) ::dlsym(g_swiftLibHandle, "HAL_openCameraHardware");
            //g_getCameraInfo = (GetCamInfo) ::dlsym(g_swiftLibHandle, "HAL_getCameraInfo");
...
{
    LOGV("HAL_getNumberOfCameras: 1");
    return 1;
}

extern "C" void HAL_getCameraInfo(int cameraId, struct CameraInfo* cameraInfo)
{
    LOGV("HAL_getCameraInfo: call getCameraInfo");
...
{
    LOGV("%s :", __func__);
    mSwiftInterface->setCallbacks(notify_cb, data_cb, data_cb_timestamp, user/*user or this*/);
}
